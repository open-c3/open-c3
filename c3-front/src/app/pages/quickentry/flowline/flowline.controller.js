(function() {
    ...
    function CiGroupController($location, $anchorScroll, $state, $http, $uibModal, treeService, ngTableParams, resoureceService, $websocket, genericService, $scope, $injector) {
        ...
        vm.describek8s = function (ticketid,type,name,namespace,kind) {
            if(  kind == 'deployment' )
            {
                vm.describedeployment( ticketid,type,name,namespace );
            }
            if(  kind == 'daemonset' )
            {
                vm.describeutil( ticketid,type,name,namespace,kind );
            }
        };
        ...
        vm.describeutil = function (ticketid,type,name,namespace,kind) {
            $uibModal.open({

...
post '/loginext/google' => sub {
    ...
    return +{ stat => $JSON::false, info => "Google login not on,Please have the admin open it" } unless $api::loginext::data{'google'}{'on'};
    ...
    return +{ stat => $JSON::false, info => sprintf "call www.googleapis.com fail, code: %s", $res->code } unless $res->is_success;

    my $dat = eval{ JSON::from_json( $res->decoded_content ) };
    return +{ stat => $JSON::false, info => sprintf "call www.googleapis.com no json" } if $@;

    my $user = $dat->{email};
    return +{ stat => $JSON::false, info => sprintf "call www.googleapis.com nofind user" } unless $user;

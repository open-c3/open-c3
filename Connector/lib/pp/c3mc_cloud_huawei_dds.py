...
class LibHuaweiDds:
    ...
    def add_tags(self, instance_id, tag_list):
        ...
        Args:
            ...
            tag_list (list): 要添加的标签列表。格式为 [TagItem, TagItem]
        """
        ...
        return self.client.batch_tag_action(request)
    ...
    def remove_tags(self, instance_id, tag_list):
        ...
        Args:
            instance_id (str): 实例id
            tag_list (list): 要删除的标签列表。格式为 [TagItem, TagItem]
        """
        request = BatchTagActionRequest()
        request.instance_id = instance_id
        request.body = BatchOperateInstanceTagRequestBody(
            tags=tag_list, action="delete"
        )
        return self.client.batch_tag_action(request)

...
`batches` VARCHAR(20) comment '什么时候生效,分批维度',  ###只允许 firsttime, thelasttime, notfirsttime, notthelasttime, always
`everyone` VARCHAR(20) comment '是否每个人都需要审批',  ###只允许 on, off
`timeout` VARCHAR(30) comment '超时时间',
`pause` VARCHAR(100) comment '需要暂停',
`create_time` TIMESTAMP DEFAULT CURRENT_TIMESTAMP comment '创建时间',
`relaxed` VARCHAR(20) comment '宽松的审批，提交人和审批人一样的时候跳过',  ###只允许 on, off, nervous

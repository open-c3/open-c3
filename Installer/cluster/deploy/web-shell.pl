...
unless( $o{version} )
{
    map { die "show version to $_ fail:$!\n" if system "sshpass -p '$config->{password}' ssh $config->{username}\@$_ 'cd $MYDan_PATH/PKG/&& ls |sort|grep ^web-shell-'"; }@ip;
    exit;
}
...
map
{
    die "rsync to $_ fail:$!\n" if system "sshpass -p '$config->{password}' rsync -av $BASE_PATH/web-shell/ $config->{username}\@$_:$MYDan_PATH/PKG/web-shell-$o{version}/ --exclude conf/ --delete";
}@ip unless $o{rollback};

map
{
    die "local.web-shell.pl to $_ fail:$!\n" if system "sshpass -p '$config->{password}' ssh $config->{username}\@$_ '$MYDan_PATH/Installer/cluster/deploy/local/web-shell.pl' -e '$o{envname}' -v '$o{version}'";
}@ip;

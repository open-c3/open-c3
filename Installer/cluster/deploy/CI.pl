...
my $Config = eval{ YAML::XS::LoadFile "$BASE_PATH/CI/config/Config" };
...
unless( $o{version} )
{
    map { die "show version to $_ fail:$!\n" if system "sshpass -p '$config->{password}' ssh $config->{username}\@$_ 'cd $MYDan_PATH/PKG/&& ls |sort|grep ^CI-'"; }@ip;
    exit;
}
...
map
{
    die "rsync to $_ fail:$!\n" if system "sshpass -p '$config->{password}' rsync -av $BASE_PATH/CI/ $config->{username}\@$_:$MYDan_PATH/PKG/CI-$o{version}/ --exclude conf/ --delete";
}@ip unless $o{rollback};

map { die "local.CI.pl to $_ fail:$!\n" if system "sshpass -p '$config->{password}' ssh $config->{username}\@$_ '$MYDan_PATH/Installer/cluster/deploy/local/CI.pl' -e '$o{envname}' -v '$o{version}'"; }@ip;

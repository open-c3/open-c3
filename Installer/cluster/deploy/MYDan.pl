...
unless( $o{version} )
{
    map { die "show version to $_ fail:$!\n" if system "sshpass -p '$config->{password}' ssh $config->{username}\@$_ 'cd $MYDan_PATH/PKG/&& ls |sort|grep ^MYDan-'"; }@ip;
    exit;
}
...
map
{
    die "rsync to $_ fail:$!\n" if system "sshpass -p '$config->{password}' rsync -av $BASE_PATH/MYDan/ $config->{username}\@$_:$MYDan_PATH/PKG/MYDan-$o{version}/ --exclude conf/ --delete";
}@ip unless $o{rollback};

map { die "local.MYDan.pl to $_ fail:$!\n" if system "sshpass -p '$config->{password}' ssh $config->{username}\@$_ '$MYDan_PATH/Installer/cluster/deploy/local/MYDan.pl' -e '$o{envname}' -v '$o{version}'"; }@ip;

...
{
    my $this = shift;
#    map{ notice( "jobxsys $this->{type} $_") }@_;
    map{ $this->_write( "INFO jobxsys $this->{type} $_" ) }@_;
}

sub err
{
    my $this = shift;
    map{ $this->_write( "ERROR jobxsys $this->{type} $_" ) }@_;
    $H->autoflush(1);
#    map{ error( "jobxsys $this->{type} $_") }@_;
}
...
sub die
{
    my $this = shift;
#    map{ error( "jobxsys $this->{type} $_") }@_;
    map{ $this->_write( "FAIL jobxsys $this->{type} $_" ) }@_;
    $H->autoflush(1);

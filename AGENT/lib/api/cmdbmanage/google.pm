...
post '/cmdbmanage/account/google' => sub {
    ...
    my $credentials = encode_base64( encode('UTF-8', $param->{credentials}) );

    my $r = eval{ 
        $api::mysql->execute(
           $param->{id}
              ? "update openc3_device_account_google set accountname='$param->{accountname}',credentials='$credentials',region='$param->{region}',note='$param->{note}',edit_user='$user' where id='$param->{id}'"
              : "insert into openc3_device_account_google (`accountname`,`credentials`,`region`,`note`,`edit_user`)values( '$param->{accountname}','$credentials', '$param->{region}','$param->{note}','$user')"
         )
    };

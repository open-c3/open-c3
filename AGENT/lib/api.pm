...
hook 'before' => sub {
    ...
    return if $uri =~ m{^/mon} || $uri =~ m{^/scripts/} || $uri =~ m{^/reload};
...
#
#    my $p = eval{ $pms->run( cookie => cookie( $cookiekey ), uri => $uri, method => request->method, map{ $_ => request->headers->{$_} }qw( appkey appname )  ) };

package OPENC3::MYDan::MonitorV3::RedisQuery;
...
BEGIN
{
    system "mkdir -p /data/open-c3-data/redis-exporter-v3/cache";
    $cmc = 1 if $ENV{C3_RedisQuery_Container};
};
...
sub getResponseProxy
{
    ...
    $content = join "\n",
        "# HELP OPEN-C3 RedisQuery debug[$debug]",
        @debug,
        $content;
...
'
<html>
    <head><title>OPEN-C3 Redis Query</title></head>
    <body>
        <h1>OPEN-C3 Redis Query</h1>
    </body>
</html>
';
...
sub run
{
                   ...
                   sub { 
                       ...
                       if( $data =~ m#/query_([a-z0-9][a-z0-9\-\.]+[a-z0-9]):(\d+)_query# )
                       {
                           ...
                           if( $data =~ m#carry_([a-zA-Z0-9+/=]+)_carry#  )
                           {
                               ...
                               warn "redis query carry data err: $@" if $@;
                           ...
                           if( $proxy{$addr} )
                           {
                               ...
                               $url = "http://$proxy{$addr}:65114/redis/metrics/query_${addr}_query/$carry/$debug";
                           }
                           else
                           {
                               my $dockername = "openc3-redis-exporter-v3-$ip-$port";
                               $dockername = $alias{$dockername} if $alias{$dockername};
                               $url = "http://$dockername:9121/metrics";
                               eval{
                                   YAML::XS::DumpFile "/data/open-c3-data/redis-exporter-v3/cache/$addr", \%exp;
                               };
    ...
    my $aliasUpdate = AnyEvent->timer(
        ...
        interval => 60,
        cb       => sub {
            my $alias = eval{ YAML::XS::LoadFile "/etc/openc3.redis-v3-exporter.alias.yml" };

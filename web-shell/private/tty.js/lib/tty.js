...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(uri, cols, rows, func) {
    return session.handleCreate(uri,cols, rows, func);
  });
...
Session.prototype.handleCreate = function(uri,cols, rows, func) {
  var self = this
    , terms = this.terms
